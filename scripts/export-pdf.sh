...
#
# Usage: scripts/export-pdf.sh <deck-dir> [out.pdf]
#   e.g. scripts/export-pdf.sh dit_ir_webinar_2026
# Renders first if the output is missing, serves it on a free local port,
# runs DeckTape, stops the server. Needs bun (for bunx decktape) and a Chrome;
# CHROME_PATH overrides auto-detection.
set -euo pipefail
cd "$(dirname "$0")/.."
